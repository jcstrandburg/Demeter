...
class Dictionary extends ArrayCollection implements \ArrayAccess
{
    ...
    public function removeCore(iterable $keys): Dictionary
    {
        $data = $this->dict;
        foreach ($keys as $key) {
            unset($data[$key]);
    ...
    {
        return new Dictionary(array_map($mapper, $this->dict));
    }
     ...
     * @return  Dictionary
     */
    public static function from(iterable $keyValuePairs): Dictionary
    {
        $dict = [];
